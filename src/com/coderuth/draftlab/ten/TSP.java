...
class TSP {
    private int weight[][], n, tour[], finalCost;
    ...
    TSP() {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                if (i != j) {
                    System.out.print("Distance from " + (i + 1) + " to " + (j + 1) + " : ");
                    weight[i][j] = s.nextInt();
                }
            }
        }
        System.out.println();
        System.out.println("Starting Node Assumed to be 1. ");
        eval();
    }
    ...
    public int COST(int currentNode, int inputSet[], int setSize) {
        ...
        int min = INF;
        int setToBePassed[] = new int[n - 1];
        for (int i = 0; i < setSize; i++) {
            int k = 0;//initialise new set
            for (int j = 0; j < setSize; j++) {
                if (inputSet[i] != inputSet[j])
                    setToBePassed[k++] = inputSet[j];
            }
            int temp = COST(inputSet[i], setToBePassed, setSize - 1);
